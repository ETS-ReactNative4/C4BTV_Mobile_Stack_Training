...
2.  Run the command "expo init test-app"
    You should see some options for app templates
...
Choose "blank"  this is only a test


3. When it's done, follow the direction on the screen to start the app
  cd test-app
  expo start
...
When the Expo client is loaded, the emulator will prompt you for permission to "draw over other apps."  Click the "OK" button.
...
![Alt](assets/app.png "App")___________